...
public:
	...
	void addAnimatedMainPlayer(std::string fileName, std::string aniJSON, int width, int height, float transparency, float x, float y, float z, float shrinkX, float shrinkY, b2World* m_physicsWorld);
	void addMainPlayer(std::string fileName, int width, int height, float transparency, float x, float y, float z, float shrinkX, float shrinkY, b2World* m_physicsWorld);
	void addMainCamera(vec4 temp, float windowWidth, float windowHeight);
	void addStaticEntity(std::string file, int width, int height, float transparency, vec3 position, int sX, int sY, b2World* m_physicsWorld);
	void addDynamicEntity(std::string file, int width, int height, float transparency, vec3 position, int sX, int sY, b2World* m_physicsWorld, int type);
	void addEntity(std::string file, int width, int height, float transparency, vec3 position);
protected:
